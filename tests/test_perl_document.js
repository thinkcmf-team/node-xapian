...
[
  ...
  },
  {
    ...
    method: 'values',
    parameters: [],
    result: function(err, result, fn) { fn(
      err == null && result.length == 2 &&
  ...
  },
  {
    ...
    action: function(objects, sync, fn) { 
      if (sync) {
        objects.db = new xapian.WritableDatabase('tmpdb', xapian.DB_CREATE_OR_OVERWRITE); 
        fn(null); 
      } else {
        new xapian.WritableDatabase('tmpdb', xapian.DB_CREATE_OR_OVERWRITE, function(err, result) {
          objects.db = result;
          fn(err);
        });
      }
